...
import { PopoveProps,PopoveState } from 'types/popover'
import './style'


const componentName = 'cl-popover'
...
class Popover extends React.Component<PopoveProps, PopoveState> {
  ...
  public render() {
    ...
    const { derivedVisible } = this.state
    const wrapperClassName = cx(`${cn}-content-wrapper`, [
      'position-' + position
    ])
    const contentClassName = cx(`${cn}-content`, [className])
    return (
      <>
        <Transition
